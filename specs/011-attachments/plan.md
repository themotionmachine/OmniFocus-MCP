...
2. Zod schema enforces max stripped string length of 69,905,068 characters (~50 MB decoded) via `.refine((val) => val.length <= 69905068, ...)` (Pattern B: `.transform()` then `.refine()`, matching `add-folder.ts` precedent)
