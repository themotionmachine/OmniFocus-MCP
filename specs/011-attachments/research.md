...
1. **Server-side (TypeScript)**: Validate base64 string using regex (`/^[A-Za-z0-9+/]*={0,2}$/`) and arithmetic size estimation (`Math.floor((length * 3) / 4) - padding`) before passing to OmniJS. This catches invalid encoding and oversized payloads early with clear error messages.
...
**Decision**: Two-tier validation in TypeScript before OmniJS execution, using arithmetic estimation of decoded size (to avoid allocating large buffers):
...
Size calculation: let `len = base64String.length`. Compute padding as the number of trailing `=` characters (`0`, `1`, or `2`), then estimate decoded bytes as `Math.floor((len * 3) / 4) - padding`. This matches `Buffer.from(base64String, 'base64').length` for valid base64 while avoiding large buffer allocations.

**Rationale**: 10 MB warning threshold aligns with OmniFocus Sync best practices. 50 MB hard limit prevents MCP server memory exhaustion. Constitution Principle V (Defensive Error Handling) mandates proactive protection. Arithmetic estimation is precise for valid base64 (including padding handling) and is safer than constructing a potentially huge `Buffer` just to measure its length.

**Alternatives considered**:
- Exact size via `Buffer.from(base64String, 'base64').length` -- rejected because it allocates a full decoded buffer, which is exactly what the limit is intended to guard against.
