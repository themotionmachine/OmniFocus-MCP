...
- **FR-001**: System MUST accept a transport text string and create all tasks, projects, and metadata described in that string within OmniFocus, returning the identifiers of every created item (both top-level tasks and all nested subtasks). The implementation collects all identifiers by accessing `flattenedTasks` on each top-level task returned by `byParsingTransportText` (which returns ALL descendants as a flat array), or equivalently by recursively walking `task.tasks` (direct children only) on each top-level task. The `byParsingTransportText` API is lenient: it silently ignores unrecognized lines and may create fewer items than the input implies; the import tool returns only the items actually created without attempting to detect skipped lines.
