...
- What happens when a task is moved to a project that is completed or dropped? The system should allow the move (OmniFocus permits this) and include a `warning` string in the per-item `BulkBatchItemResult` indicating the target's inactive status (e.g., "Target project is completed"). The `warning` field is optional and only populated when the operation succeeds but a non-fatal concern exists. This is a new field unique to bulk-tools; no existing batch schema has a warning field, so this is a net-new addition to the batch result pattern
...
- What happens when the `relativeTo` task/section in a position parameter does not exist? Since the position (including `relativeTo`) is shared across all items in the batch, the operation returns a top-level RELATIVE_TARGET_NOT_FOUND error before processing any items, similar to TARGET_NOT_FOUND. The `relativeTo` sibling is resolved once at the beginning of the OmniJS script as part of target pre-validation
...
- What happens when `batch_update_tasks` specifies the same tag in both `addTags` and `removeTags`? Removals are processed before additions (FR-014), so the net result is the tag is added. This is not a validation error; the ordering rule resolves the overlap deterministically
- What happens when `batch_update_tasks` specifies empty arrays (`addTags: []` or `removeTags: []`)? Empty arrays do not count as specified properties for the "at least one property" validation (FR-013). The arrays must contain at least one element if provided, consistent with the `remove_tags` tool pattern
- What happens when `batch_update_tasks` specifies `clearDueDate: false` (or any `clearX: false`)? Only `clearX: true` is treated as a substantive property specification. `clearX: false` is a semantic no-op and does not satisfy the "at least one property" requirement (FR-013). The validation checks for `clearX === true`, not merely `clearX !== undefined`
- What happens when `convert_tasks_to_projects` specifies both `targetFolderId` and `targetFolderName`? `targetFolderId` takes precedence; `targetFolderName` is ignored. This is consistent with the `create_project` and `move_project` tools
- What happens when `move_tasks` or `duplicate_tasks` specifies multiple target types in the position (e.g., both `projectId` and `inbox: true`)? The system returns a validation error requiring exactly one target type. Only one of `projectId`, `projectName`, `taskId`, `taskName`, or `inbox` may be specified
- What happens when `duplicate_tasks` specifies a source task as the `relativeTo` target with `beginning` or `ending` placement (i.e., duplicating a task into itself as a subtask)? OmniFocus throws an exception for circular parent-child relationships; the server catches it per-item and returns OPERATION_FAILED with the native error message. For `before`/`after` placement where `relativeTo` matches a source task, the operation is valid (creates a sibling copy adjacent to the original) and proceeds normally
- What happens when all items in a batch fail resolution (e.g., all IDs are invalid)? The outer response is `success: true` with `summary: { total: N, succeeded: 0, failed: N }` and per-item results showing individual NOT_FOUND errors. Top-level `success: false` is reserved for structural/pre-validation failures (TARGET_NOT_FOUND, VALIDATION_ERROR, OmniFocus unreachable), not for batches where all individual items happen to fail. This matches the established `dropItems` and `markComplete` pattern
- Does `convertTasksToProjects()` have a subtask depth limitation? No. The OmniJS native function handles arbitrarily deep subtask hierarchies. All subtasks at any nesting level become project child tasks preserving their relative hierarchy. The spec does not impose any depth constraint beyond what OmniFocus natively supports
...
- **Position**: A placement specification consisting of a placement type (beginning, ending, before, after) and an optional relative-to identifier for before/after placements. Task operations accept positions targeting projects, tasks (as subtask), or the inbox; `placement` defaults to `ending` when omitted. Section operations accept positions targeting folders or the library root only (not inbox or tasks); `placement` is required (no default), consistent with the existing `move_folder` tool's `PositionSchema`. This asymmetry is intentional: section operations follow the established folder-tools contract where placement is always explicit, while task operations add a default for convenience since task move/duplicate operations are more frequent.
...
- **Batch Result**: A per-item outcome containing success/failure status, the item's ID, any error code and message, disambiguation candidates when applicable, and an optional `warning` string for non-fatal concerns (e.g., target is inactive). For duplicate and convert operations, the result also includes `newId` and `newName` of the created item
- **Property Update Set**: The collection of property changes to apply uniformly to all tasks in a batch-update operation. Each property is optional; only specified properties are modified. Supported properties: `flagged` (boolean), `dueDate` (ISO 8601 string), `deferDate` (ISO 8601 string), `addTags` (array of tag names/IDs), `removeTags` (array of tag names/IDs), `note` (string, appended), `estimatedMinutes` (non-negative number, min 0), `plannedDate` (ISO 8601 string, v4.7+). Clear flags: `clearDueDate`, `clearDeferDate`, `clearEstimatedMinutes`, `clearPlannedDate` (all boolean). Tag removals are processed before additions (FR-014). Date string fields (`dueDate`, `deferDate`, `plannedDate`) use `z.string()` without Zod-level format validation, consistent with all existing tool contracts in the codebase; format validation is delegated to the OmniJS/OmniFocus application layer
