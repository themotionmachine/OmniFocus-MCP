...
describe('SearchTagsInputSchema', () => {
  ...
  it('should reject whitespace-only query after trimming', () => {
    expect(SearchTagsInputSchema.safeParse({ query: '   ' }).success).toBe(false);
    expect(SearchTagsInputSchema.safeParse({ query: '\t\n' }).success).toBe(false);
  });

  it('should reject query exceeding 1000 characters', () => {
    const longQuery = 'a'.repeat(1001);
    expect(SearchTagsInputSchema.safeParse({ query: longQuery }).success).toBe(false);
  });

  it('should accept query exactly at 1000 characters', () => {
    const maxQuery = 'a'.repeat(1000);
    expect(SearchTagsInputSchema.safeParse({ query: maxQuery }).success).toBe(true);
  });
