...
describe('SearchProjectsInputSchema', () => {
  ...
  it('should reject whitespace-only query after trimming (FR-016)', () => {
    expect(SearchProjectsInputSchema.safeParse({ query: '   ' }).success).toBe(false);
    expect(SearchProjectsInputSchema.safeParse({ query: '\t\n' }).success).toBe(false);
  });

  it('should reject query exceeding 1000 characters', () => {
    const longQuery = 'a'.repeat(1001);
    expect(SearchProjectsInputSchema.safeParse({ query: longQuery }).success).toBe(false);
  });

  it('should accept query exactly at 1000 characters', () => {
    const maxQuery = 'a'.repeat(1000);
    expect(SearchProjectsInputSchema.safeParse({ query: maxQuery }).success).toBe(true);
  });
