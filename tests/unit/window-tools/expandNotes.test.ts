...
import {
  expandNotes,
  generateExpandNotesScript
} from '../../../src/tools/primitives/expandNotes.js';
...
// T041: Unit tests for expandNotes primitive
...
describe('expandNotes', () => {
  ...
  it('should return NOT_FOUND when item does not exist', async () => {
    ...
    const result = await expandNotes({ items: [{ id: 'bad' }] });
    expect(result.success).toBe(true);
  ...
  it('should return NODE_NOT_FOUND when item not visible', async () => {
    ...
    const result = await expandNotes({ items: [{ id: 'task-hidden' }] });
    expect(result.success).toBe(true);
  ...
  it('should return DISAMBIGUATION_REQUIRED', async () => {
    ...
    const result = await expandNotes({ items: [{ name: 'Review' }] });
    expect(result.success).toBe(true);
  ...
  it('should fail on version guard', async () => {
    vi.mocked(executeOmniJS).mockResolvedValue({
      success: false,
      error: 'expand_notes requires OmniFocus 4.0'
    });
    const result = await expandNotes({ items: [{ id: 'task-abc' }] });
  ...
  it('should handle batch partial failures', async () => {
    ...
    const result = await expandNotes({ items: [{ id: 'good' }, { id: 'bad' }] });
    expect(result.success).toBe(true);
    if (result.success) {
...
describe('generateExpandNotesScript', () => {
  it('should produce valid OmniJS script', () => {
    const script = generateExpandNotesScript({ items: [{ id: 'task-abc' }] });
    expect(script).toMatch(/^\(function\(\)/);
    expect(script).toContain('.expandNote(');
