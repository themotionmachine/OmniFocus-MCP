...
import {
  parseCssHexToRgba,
  setPerspectiveIcon
} from '../../../src/tools/primitives/setPerspectiveIcon.js';
...
describe('parseCssHexToRgba', () => {
  it('converts 3-digit #RGB to RGBA floats', () => {
    const result = parseCssHexToRgba('#F00');
  ...
  it('converts 4-digit #RGBA to RGBA floats with alpha', () => {
    const result = parseCssHexToRgba('#F008');
    expect(result.r).toBe(1);
    expect(result.g).toBe(0);
    expect(result.b).toBe(0);
    // 0x88 / 255 ≈ 0.533
  ...
  it('converts 6-digit #RRGGBB to RGBA floats', () => {
    const result = parseCssHexToRgba('#FF8040');
    expect(result.r).toBe(1);
    // 0x80 / 255 ≈ 0.502
  ...
  it('converts 8-digit #RRGGBBAA to RGBA floats with alpha', () => {
    const result = parseCssHexToRgba('#FF000080');
    expect(result.r).toBe(1);
    expect(result.g).toBe(0);
    expect(result.b).toBe(0);
    // 0x80 / 255 ≈ 0.502
  ...
  it('converts #000000 (black) to all-zero RGB with full alpha', () => {
    const result = parseCssHexToRgba('#000000');
  ...
  it('converts #FFFFFF (white) to all-one RGBA floats', () => {
    const result = parseCssHexToRgba('#FFFFFF');
    expect(result.r).toBe(1);
  ...
  it('returns NaN components for invalid hex digits', () => {
    // #GGGGGG has length 6 but all G's are not valid hex digits
    const result = parseCssHexToRgba('#GGGGGG');
    expect(Number.isNaN(result.r)).toBe(true);
    expect(Number.isNaN(result.g)).toBe(true);
