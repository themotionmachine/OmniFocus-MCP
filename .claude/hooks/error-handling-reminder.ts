...
function getFileCategory(filePath: string): 'frontend' | 'infrastructure' | 'other' {
  ...
  if (
    ...
    filePath.includes('/figma-make-prototype/')
  )
    return 'frontend';

  // Infrastructure detection (CDK)
  if (filePath.includes('/infrastructure/') || filePath.includes('/cdk/')) return 'infrastructure';
...
async function main() {
  try {
    ...
    const needsAttention = analysisResults.some(
      ({ analysis }) =>
        analysis.hasTryCatch || analysis.hasAsync || analysis.hasApiCall || analysis.hasEventHandler
    );
