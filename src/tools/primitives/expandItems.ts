import {
  type ExpandItemsInput,
  type ExpandItemsResponse,
  ExpandItemsResponseSchema
} from '../../contracts/window-tools/expand-items.js';
import { executeOmniJS } from '../../utils/scriptExecution.js';
import { generateResolveItemSnippet } from './shared/resolveItemSnippet.js';

/**
 * Expand one or more outline nodes in OmniFocus.
 *
 * Shows children of targeted items. Optional `completely` parameter
 * expands all descendants recursively.
 ...
 */
export async function expandItems(params: ExpandItemsInput): Promise<ExpandItemsResponse> {
  const script = generateExpandItemsScript(params);
  const result = await executeOmniJS(script);
  if (!result) return { success: false, error: 'OmniJS returned empty result' };
  return ExpandItemsResponseSchema.parse(result);
}

/**
 * Generate OmniJS script to expand outline nodes.
 * Exported for manual testing in OmniFocus Script Editor.
 */
export function generateExpandItemsScript(params: ExpandItemsInput): string {
  ...
  try {
    if (!app.userVersion.atLeast(new Version('4.0'))) {
      return JSON.stringify({
        success: false,
        error: 'expand_items requires OmniFocus 4.0 or later. Current version: ' + app.userVersion.versionString
      });
    }
