import {
  type CollapseNotesInput,
  type CollapseNotesResponse,
  CollapseNotesResponseSchema
} from '../../contracts/window-tools/collapse-notes.js';
import { executeOmniJS } from '../../utils/scriptExecution.js';
import { generateResolveItemSnippet } from './shared/resolveItemSnippet.js';

/**
 * Collapse notes on one or more outline nodes in OmniFocus.
 *
 * Hides note content. Optional `completely` parameter collapses
 * notes on all descendants recursively.
 ...
 */
export async function collapseNotes(params: CollapseNotesInput): Promise<CollapseNotesResponse> {
  const script = generateCollapseNotesScript(params);
  const result = await executeOmniJS(script);
  if (!result) return { success: false, error: 'OmniJS returned empty result' };
  return CollapseNotesResponseSchema.parse(result);
}

/**
 * Generate OmniJS script to collapse notes on outline nodes.
 * Exported for manual testing in OmniFocus Script Editor.
 */
export function generateCollapseNotesScript(params: CollapseNotesInput): string {
  ...
  try {
    if (!app.userVersion.atLeast(new Version('4.0'))) {
      return JSON.stringify({
        success: false,
        error: 'collapse_notes requires OmniFocus 4.0 or later. Current version: ' + app.userVersion.versionString
      });
    }
...
${generateResolveItemSnippet({ includeNote: true })}
    ...
    for (var i = 0; i < itemIdentifiers.length; i++) {
      ...
      if (!resolved.note || resolved.note.length === 0) {
        results.push({
          itemId: resolved.id,
          itemName: resolved.name,
          itemType: resolved.type,
