...
export async function exportPerspective(
  ...
  logger.debug('Exporting perspective', 'exportPerspective', {
    name: params.name,
  ...
  logger.debug('exportPerspective script returned', 'exportPerspective');
  return ExportPerspectiveResponseSchema.parse(result);
}
...
export function generateExportPerspectiveScript(params: ExportPerspectiveInput): string {
  ...
  try {
    ...
    if (saveTo && saveTo.length > 0) {
      ...
      try {
        var encodedPath = encodeURI(saveTo).replace(/#/g, "%23");
        var dirURL = URL.fromString("file://" + encodedPath + "/");
