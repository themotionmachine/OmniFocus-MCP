import {
  type ExpandNotesInput,
  type ExpandNotesResponse,
  ExpandNotesResponseSchema
} from '../../contracts/window-tools/expand-notes.js';
import { executeOmniJS } from '../../utils/scriptExecution.js';
import { generateResolveItemSnippet } from './shared/resolveItemSnippet.js';

/**
 * Expand notes on one or more outline nodes in OmniFocus.
 *
 * Shows note content inline. Optional `completely` parameter expands
 ...
 */
export async function expandNotes(params: ExpandNotesInput): Promise<ExpandNotesResponse> {
  const script = generateExpandNotesScript(params);
  const result = await executeOmniJS(script);
  if (!result) return { success: false, error: 'OmniJS returned empty result' };
  return ExpandNotesResponseSchema.parse(result);
}

/**
 * Generate OmniJS script to expand notes on outline nodes.
 * Exported for manual testing in OmniFocus Script Editor.
 */
export function generateExpandNotesScript(params: ExpandNotesInput): string {
  ...
  try {
    if (!app.userVersion.atLeast(new Version('4.0'))) {
      return JSON.stringify({
        success: false,
        error: 'expand_notes requires OmniFocus 4.0 or later. Current version: ' + app.userVersion.versionString
      });
    }
