import type {
  MoveSectionsInput,
  MoveSectionsResponse
} from '../../contracts/bulk-tools/move-sections.js';
import { executeOmniJS } from '../../utils/scriptExecution.js';

/**
 * Move 1-100 sections (folders and/or projects) to a new location in OmniFocus.
 *
 * Item resolution probes Folder.byIdentifier first, then Project.byIdentifier (AD-14).
 * Name-based lookup searches both flattenedFolders and flattenedProjects.
 * Post-move verification reads parentFolder after moveSections() (AD-12).
 */
export async function moveSections(params: MoveSectionsInput): Promise<MoveSectionsResponse> {
  const script = generateMoveSectionsScript(params);
  const result = await executeOmniJS(script);
  return result as MoveSectionsResponse;
}

/**
 * Generate OmniJS script to move sections to a target location.
 * Exported for manual testing in OmniFocus Script Editor.
 */
export function generateMoveSectionsScript(params: MoveSectionsInput): string {
  ...
  try {
    ...
    itemIdentifiers.forEach(function(identifier) {
      ...
      try {
        ...
        // Perform the move
        moveSections([item], targetPosition);

        // Post-move verification (AD-12) - read parentFolder
