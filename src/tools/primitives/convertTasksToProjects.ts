...
export async function convertTasksToProjects(
  ...
  return ConvertTasksToProjectsResponseSchema.parse(result);
}
  ...
  try {
    ...
    if (folderLookup.targetFolderId) {
      ...
      if (folderMatches.length > 1) {
        return JSON.stringify({ success: false, error: "Multiple folders match '" + folderLookup.targetFolderName + "'. Use targetFolderId.", code: 'TARGET_DISAMBIGUATION_REQUIRED' });
      }
