    ...
    .array(
      z.object({
        ...
        deferDate: z
          .string()
          .optional()
          .describe('The defer date in ISO format (YYYY-MM-DD or full ISO date)'),
        ...
        projectName: z
          .string()
          .optional()
          .describe('For tasks: The name of the project to add the task to'),
        parentTaskId: z.string().optional().describe('For tasks: ID of the parent task'),
        parentTaskName: z
          .string()
          .optional()
          .describe('For tasks: Name of the parent task (scoped to project when provided)'),
        tempId: z
          .string()
          .optional()
          .describe('For tasks: Temporary ID for within-batch references'),
        parentTempId: z
          .string()
          .optional()
          .describe("For tasks: Reference to parent's tempId within the batch"),
        hierarchyLevel: z
          .number()
          .int()
          .min(0)
          .optional()
          .describe('Optional ordering hint (0=root, 1=child, ...)'),

        // Project-specific properties
        folderName: z
          .string()
          .optional()
          .describe('For projects: The name of the folder to add the project to'),
        sequential: z
          .boolean()
          .optional()
          .describe('For projects: Whether tasks in the project should be sequential')
      })
    )
    .describe('Array of items (tasks or projects) to add'),
  createSequentially: z
    .boolean()
    .optional()
    .describe(
        ...
        result.results && result.results.length > 0
          ? result.results
              .map((r, index) => {
                ...
                if (!inputItem) return `- ❓ Unknown item at index ${index}`;
                return r.success
                  ? `- ✅ ${inputItem.type}: "${inputItem.name}"`
                  : `- ❌ ${inputItem.type}: "${inputItem.name}" - Error: ${r?.error || 'Unknown error'}`;
              })
              .join('\\n')
