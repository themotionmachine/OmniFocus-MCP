...
export const schema = z.object({
  ...
  includeMetadata: z
    .boolean()
    .optional()
    .describe('Include additional metadata like project names, tags, dates. Default: true'),

  fields: z
    .array(z.string())
    .optional()
    .describe(
      'Specific fields to include in the response. Reduces response size. Available fields: id, name, note, flagged, dueDate, deferDate, completionDate, taskStatus, projectName, tagNames, estimatedMinutes'
    )
});
  ...
  try {
    ...
    if (result.success) {
      ...
      if (items.length === 0) {
        ...
        // Format each item
        items.forEach((item: PerspectiveItem) => {
          const parts = [];
          ...
          if (trimmedNote) {
            ...
            const notePreview = firstLine.substring(0, 80);
            const ellipsis = trimmedNote.length > 80 || trimmedNote.includes('\n') ? '...' : '';
