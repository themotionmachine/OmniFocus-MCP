...
export const schema = z.object({
  ...
  filters: z.object({
    ...
    dueWithin: z.union([z.number(), z.string()]).optional().describe("Due by the end of the given day; overdue included"),
    deferredUntil: z.union([z.number(), z.string()]).optional().describe("Defer date by the end of the given day; past defer dates included"),
    plannedWithin: z.union([z.number(), z.string()]).optional().describe("Planned by the end of the given day; earlier included"),
...
function formatTasks(tasks: any[]): string {
  return tasks.map(task => {
    ...
    if (task.repetitionRule) {
      ...
      // "Fixed" alone hides which date the schedule counts from; a due-anchored
      // fixed repeat on an item with no due date grows one on completion.
      const anchor = task.repetitionAnchor
        ? `, from ${String(task.repetitionAnchor).replace(/Date$/, '').toLowerCase()}`
        : '';
      const catchUp = task.catchUpAutomatically ? ', catch up' : '';
      parts.push(`[rule: ${task.repetitionRule}${method}${anchor}${catchUp}]`);
