 ...
 *
 * ## Per-Item Resolution
 * Resolves each item individually via `tree.nodeForObject(object)` to enable
 * per-item error tracking. Items not visible in the current view are reported as
 * NODE_NOT_FOUND in per-item results.
 *
 * ## Error Conditions
