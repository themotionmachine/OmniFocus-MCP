 ...
 */
export const ExportPerspectiveInputSchema = PerspectiveIdentifierSchema.safeExtend({
  saveTo: z
    .string()
    .min(1)
    .refine((p) => p.startsWith('/'), {
      message: 'saveTo must be an absolute path (start with /)'
    })
    .refine((p) => !p.split('/').includes('..'), {
      message: 'saveTo must not contain ".." path traversal segments'
    })
    .optional()
    .describe(
      'Absolute directory path to save the exported .ofocus-perspective file. When omitted, export metadata is returned in the response.'
    )
});
